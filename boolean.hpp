...
public:
    ...
    {
        boolean_function bf_from_anf(v);
        return boolean_function(bf_from_anf.anf());
    }
    ...
    bool operator >= (const boolean_function& rhs) const
    {
        ...
        if (size() == rhs.size())
        {
            for (size_t i = 0; i < rhs.size(); ++i)
            {
                ...
                if (!flag && bf[i] > rhs[i])
                {
                    throw std::logic_error("incomparable functions");
                }
            }
            return flag;
        }
    ...
    {
        std::vector<value_type> coefficients = bf;
        size_t two = 2;
        for (size_t i = 1; i < dimension() + 1; ++i)
        {
            for (size_t s = 0; s < size() / two ; ++s)
            {
                for (size_t l = 0; l < two / 2; ++l)
                {
                    coefficients[two * s + two / 2 + l] = (coefficients[two * s + l] + coefficients[two * s + two / 2 + l]) % 2;
                }
            }
            two *= 2;
        }
